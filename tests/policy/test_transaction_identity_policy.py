...
def test_tid001_first_automatic_logical_turn(session_auto_ready):
    ...
    txn = turn_transaction.get_or_begin(sess)
...
def test_tid002_ask_continuation_reuses_identity(session_auto_ready):
    ...
    sess = session_auto_ready
    first = turn_transaction.get_or_begin(sess)
    turn_transaction.mark_waiting_for_player(sess, first.transaction_id)

    second = turn_transaction.get_or_begin(sess)
...
def test_tid004_roll_view_carries_identity(session_auto_ready):
    ...
    active = turn_transaction.get_active(sess)
...
def test_tid005_provider_retry_does_not_create_attempt(session_auto_ready):
    ...
    again = turn_transaction.get_or_begin(sess)
...
def test_tid007_stale_id_cannot_clear_newer(session_auto_ready):
    ...
    sess = session_auto_ready
    old = turn_transaction.get_or_begin(sess)
    turn_transaction.clear(sess, old.transaction_id)

    new = turn_transaction.get_or_begin(sess)
    turn_transaction.clear(sess, old.transaction_id)   # 낡은 ID
...
def test_tid008_runtime_only_serialization(wired_bot, session_auto_ready):
    ...
    sess = session_auto_ready
    turn_transaction.get_or_begin(sess)
