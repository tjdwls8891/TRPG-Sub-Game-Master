...
class SystemCog(commands.Cog):
    ...
    async def show_commands(self, ctx):
        ...
        embed.add_field(name="🎬  세션 관리", value=(
            "**세션 열기는 GM 스페이스의 [세션 열기] 버튼을 쓰십시오.**\n"
            "`!새세션 [시나리오명]` — (수동) 채널 3종 생성 + 룰북 캐시 업로드\n"
            "`!시작` — 시작 상황 삼지선다 제시 → 선택 시 인트로 재생 (1회 한정)\n"
            "`!소개` — 인트로·캐릭터 생성 안내를 게임 채널에 스트리밍\n"
            "`!세션종료` — 게임 채널 잠금 · 캐시 파기 · 과금 정산"
        ), inline=False)

        embed.add_field(name="🧙  캐릭터 설정", value=(
            "`!참가 [이름]` — PC로 참가 → **게임 채널에 프로필 생성 UI 자동 시작**\n"
            "　└ 질문에 답하고 선택지를 고르면 완성됩니다. 아래 명령은 사후 수정용입니다.\n"
        ...
        embed.add_field(name="🤖  GM  (`!자동` 그룹)", value=(
            ...
            "　└ 4층위: 판단(유형 결정) → 지시(묘사 지시) → 묘사(출력) → 추출(수치)\n"
            "　└ 진행 유형: `ASK` 질문 · `NARRATE` 즉답 · `ROLL` 판정 · `PROCEED` 턴 진행"
        ), inline=False)
        ...
        embed.add_field(name="🖥️  버튼 인터페이스 (명령어 아님)", value=(
            "**디스플레이 채널** — 세션 상태 표기 + 조작\n"
            "　└ 되감기 · 턴 재시작 · TTS/이미지/BGM/효과음 토글 · 볼륨 · 세션 오픈\n"
            "**GM 스페이스** — 서버 홈\n"
            "　└ 계정 등록 · 세션 열기 · 사전 프로필 관리(DM) · 잉크 충전\n"
            "　└ 명전 등록/숨기기 · 월드보드 공개 · 내 통계(DM) · 보드 갱신"
        ), inline=False)
